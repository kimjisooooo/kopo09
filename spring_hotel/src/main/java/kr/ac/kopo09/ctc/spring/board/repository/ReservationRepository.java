...
public interface ReservationRepository extends JpaRepository<ReservationForm, Long> {
	// JPQL을 사용한 방식
    @Query("SELECT r FROM ReservationForm r WHERE r.checkInDate = :checkInDate AND r.hotel.type = :type")
    ReservationForm findByCheckInDateAndHotelType(@Param("checkInDate") Date checkInDate, @Param("type") int type);
