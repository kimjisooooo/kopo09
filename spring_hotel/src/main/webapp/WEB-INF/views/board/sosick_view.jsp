...
<body>
	<center>
		<table border="1">
			...
			<tr>
				<td>작성일</td>
				<td><fmt:formatDate value="${sosickEntity.date}" pattern="yyyy년 MM월 dd일" /></td>
			</tr>
			<tr>
				<td>내용</td>
         ...
         <tbody>
            <c:forEach var="comment" items="${sosickCommentDto}">
               <tr>
                  ...
                  <td>${comment.name}</td>
                  <td>
                     <c:if test="${comment.depth != null && comment.depth > 0}">
                  ...
                  <td>
                        <form action="${pageContext.request.contextPath}/comment_to_board" method="post">
                           <input type="hidden" name="articleid" value="${id}">
                           <input type="hidden" name="id" value="${userId}">
                           <input type="hidden" name="parentCommentId" value="${comment.id}">
                           <input type="text" name="title" placeholder="대댓글 입력"> 
